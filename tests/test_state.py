...
class TestState(unittest.TestCase):
    ...
    def test_random(self):
        ...
        for _ in range(100):
            ...
            total_reward = 0
            self.assertTrue(max(state['real_obs'].flatten()) <= 1.0, "Out of max bound state")
            self.assertTrue(min(state['real_obs'].flatten()) >= 0.0, "Out of min bound state")
            self.assertTrue(not np.isnan(state['real_obs']).any(), "NaN inside state rep!")
            self.assertTrue(not np.isinf(state['real_obs']).any(), "Inf inside state rep!")
